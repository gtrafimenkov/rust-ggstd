...
fn test_hmac() {
    for (i, tt) in HMAC_TESTS.iter().enumerate() {
        ...
        for j in 0..4 {
            ...
            // 			if j == 1 {
            // 				h = New(fn() hash.Hash { return justHash{tt.hash()} }, tt.key)
            // 			}
        }
    }
}
...
// 		}
// 	}()
// 	New(fn() hash.Hash { return sha }, []byte("bytes"))
// }
...
// fn BenchmarkHMACSHA256_1K(b *testing.B) {
// 	key := make([]byte, 32)
// 	buf := make([]byte, 1024)
// 	h := New("sha256", key)
...
// fn BenchmarkHMACSHA256_32(b *testing.B) {
// 	key := make([]byte, 32)
// 	buf := make([]byte, 32)
// 	h := New("sha256", key)
// 	b.SetBytes(int64(len(buf)))
...
// fn BenchmarkNewWriteSum(b *testing.B) {
// 	buf := make([]byte, 32)
// 	b.SetBytes(int64(len(buf)))
// 	for i := 0; i < b.N; i++ {
// 		h := New("sha256", make([]byte, 32))
// 		h.Write(buf)
// 		mac := h.sum(&[])
// 		buf[0] = mac[0]
// 	}
