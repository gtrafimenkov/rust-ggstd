...
// //
// // Deprecated: Use utf16_from_string instead.
// fn StringToUTF16(s string) []uint16 {
// 	a, err := utf16_from_string(s)
// 	if err != nil {
...
/// utf16_from_string returns the UTF-16 encoding of the UTF-8 string
/// s, with a terminating NUL added.
pub fn utf16_from_string(s: &str) -> Vec<u16> {
    // ggstd: go code handled following:
    //
    //   If s contains a NUL byte at any
    ...
    //   }
    //
    // In rust we don't check for NUL byte because if s is valid utf-8 (as it should be),
    // there will be no NUL character.
    OsStr::new(s).encode_wide().chain(once(0)).collect()
}

/// utf16_to_string returns the UTF-8 encoding of the UTF-16 sequence s,
/// with a terminating NUL removed.
pub fn utf16_to_string(s: &[u16]) -> String {
    let mut s = s;
    // Find NUL terminator.
    for len in 0..s.len() {
        if s[len] == 0 {
...
pub fn utf16_ptr_to_string(p: *const u16) -> String {
    ...
    // Find NUL terminator.
    let mut len = 0;
    while len < std::isize::MAX {
        unsafe {
            if *p.offset(len) == 0 {
                slice = std::slice::from_raw_parts(p, len as usize);
                break;
            }
        }
        len += 1;
...
// fn StringToUTF16Ptr(s string) *uint16 { return &StringToUTF16(s)[0] }

// // UTF16PtrFromString returns pointer to the UTF-16 encoding of
// // the UTF-8 string s, with a terminating NUL added. If s
// // contains a NUL byte at any location, it returns (nil, EINVAL).
// fn UTF16PtrFromString(s string) (*uint16, error) {
// 	a, err := utf16_from_string(s)
// 	if err != nil {
// 		return nil, err
// 	}
...
// fn langid(pri, sub uint16) uint32 { return uint32(sub)<<10 | uint32(pri) }
...
// fn FormatMessage(flags uint32, msgsrc uint32, msgid uint32, langid uint32, buf []uint16, args *byte) (n uint32, err error) {
// 	return formatMessage(flags, uintptr(msgsrc), msgid, langid, buf, args)
// }

// fn (e Errno) Error() string {
...
// fn (e Errno) Is(target error) bool {
// 	switch target {
...
// fn (e Errno) Temporary() bool {
// 	return e == EINTR || e == EMFILE || e.Timeout()
// }

// fn (e Errno) Timeout() bool {
// 	return e == EAGAIN || e == EWOULDBLOCK || e == ETIMEDOUT
// }

// // Implemented in runtime/syscall_windows.go.
// fn compileCallback(fn any, cleanstack bool) uintptr
...
// // for these callbacks is never released.
// // Between NewCallback and NewCallbackCDecl, at least 1024 callbacks can always be created.
// fn NewCallbackCDecl(fn any) uintptr {
...
// fn makeInheritSa() *SecurityAttributes {
...
// fn Open(path string, mode int, perm uint32) (fd Handle, err error) {
// 	if len(path) == 0 {
...
// fn Read(fd Handle, p []byte) (n int, err error) {
// 	var done uint32
...
// fn Write(fd Handle, p []byte) (n int, err error) {
// 	var done uint32
...
// fn ReadFile(fd Handle, p []byte, done *uint32, overlapped *Overlapped) error {
...
// fn WriteFile(fd Handle, p []byte, done *uint32, overlapped *Overlapped) error {
// 	if race.Enabled {
...
// fn getStdHandle(h int) (fd Handle) {
// 	r, _ := GetStdHandle(h)
// 	return r
// }
...
// fn Chdir(path string) (err error) {
// 	pathp, err := UTF16PtrFromString(path)
// 	if err != nil {
// 		return err
// 	}
...
// fn Mkdir(path string, mode uint32) (err error) {
// 	pathp, err := UTF16PtrFromString(path)
// 	if err != nil {
// 		return err
// 	}
...
// fn Rmdir(path string) (err error) {
// 	pathp, err := UTF16PtrFromString(path)
// 	if err != nil {
// 		return err
// 	}
...
// fn Unlink(path string) (err error) {
// 	pathp, err := UTF16PtrFromString(path)
// 	if err != nil {
// 		return err
// 	}
...
// fn Rename(oldpath, newpath string) (err error) {
...
// fn ComputerName() (name string, err error) {
...
// fn Gettimeofday(tv *Timeval) (err error) {
// 	var ft Filetime
...
// fn Utimes(path string, tv []Timeval) (err error) {
// 	if len(tv) != 2 {
// 		return EINVAL
// 	}
...
// fn Chmod(path string, mode uint32) (err error) {
...
// fn LoadCancelIoEx() error {
// 	return procCancelIoEx.Find()
// }

// fn LoadSetFileCompletionNotificationModes() error {
...
// fn (sa *SockaddrInet4) sockaddr() (unsafe.Pointer, int32, error) {
...
// fn (sa *SockaddrInet6) sockaddr() (unsafe.Pointer, int32, error) {
// 	if sa.Port < 0 || sa.Port > 0xFFFF {
// 		return nil, 0, EINVAL
// 	}
...
// fn (sa *SockaddrUnix) sockaddr() (unsafe.Pointer, int32, error) {
// 	name := sa.Name
// 	n := len(name)
...
// fn (rsa *RawSockaddrAny) Sockaddr() (Sockaddr, error) {
...
// fn Socket(domain, typ, proto int) (fd Handle, err error) {
...
// fn SetsockoptInt(fd Handle, level, opt int, value int) (err error) {
// 	v := int32(value)
// 	return Setsockopt(fd, int32(level), int32(opt), (*byte)(unsafe.Pointer(&v)), int32(unsafe.Sizeof(v)))
// }

// fn Bind(fd Handle, sa Sockaddr) (err error) {
...
// fn Connect(fd Handle, sa Sockaddr) (err error) {
// 	ptr, n, err := sa.sockaddr()
// 	if err != nil {
// 		return err
// 	}
...
// fn Getsockname(fd Handle) (sa Sockaddr, err error) {
...
// fn Getpeername(fd Handle) (sa Sockaddr, err error) {
// 	var rsa RawSockaddrAny
// 	l := int32(unsafe.Sizeof(rsa))
...
// fn Listen(s Handle, n int) (err error) {
// 	return listen(s, int32(n))
// }

// fn Shutdown(fd Handle, how int) (err error) {
// 	return shutdown(fd, int32(how))
// }

// fn WSASendto(s Handle, bufs *WSABuf, bufcnt uint32, sent *uint32, flags uint32, to Sockaddr, overlapped *Overlapped, croutine *byte) (err error) {
...
// fn wsaSendtoInet4(s Handle, bufs *WSABuf, bufcnt uint32, sent *uint32, flags uint32, to *SockaddrInet4, overlapped *Overlapped, croutine *byte) (err error) {
// 	rsa, len, err := to.sockaddr()
// 	if err != nil {
// 		return err
// 	}
...
// fn wsaSendtoInet6(s Handle, bufs *WSABuf, bufcnt uint32, sent *uint32, flags uint32, to *SockaddrInet6, overlapped *Overlapped, croutine *byte) (err error) {
// 	rsa, len, err := to.sockaddr()
// 	if err != nil {
// 		return err
// 	}
...
// fn LoadConnectEx() error {
// 	connectExFunc.once.Do(fn() {
// 		var s Handle
...
// fn connectEx(s Handle, name unsafe.Pointer, namelen int32, sendBuf *byte, sendDataLen uint32, bytesSent *uint32, overlapped *Overlapped) (err error) {
...
// fn ConnectEx(fd Handle, sa Sockaddr, sendBuf *byte, sendDataLen uint32, bytesSent *uint32, overlapped *Overlapped) error {
// 	err := LoadConnectEx()
// 	if err != nil {
...
// fn (w WaitStatus) Exited() bool { return true }

// fn (w WaitStatus) ExitStatus() int { return int(w.ExitCode) }

// fn (w WaitStatus) Signal() Signal { return -1 }

// fn (w WaitStatus) CoreDump() bool { return false }

// fn (w WaitStatus) Stopped() bool { return false }

// fn (w WaitStatus) Continued() bool { return false }

// fn (w WaitStatus) StopSignal() Signal { return -1 }

// fn (w WaitStatus) Signaled() bool { return false }
...
// fn Accept(fd Handle) (nfd Handle, sa Sockaddr, err error) { return 0, nil, EWINDOWS }
// fn Recvfrom(fd Handle, p []byte, flags int) (n int, from Sockaddr, err error) {
// 	return 0, nil, EWINDOWS
// }
// fn Sendto(fd Handle, p []byte, flags int, to Sockaddr) (err error)       { return EWINDOWS }
// fn SetsockoptTimeval(fd Handle, level, opt int, tv *Timeval) (err error) { return EWINDOWS }
...
// fn GetsockoptInt(fd Handle, level, opt int) (int, error) { return -1, EWINDOWS }

// fn SetsockoptLinger(fd Handle, level, opt int, l *Linger) (err error) {
// 	sys := sysLinger{Onoff: uint16(l.Onoff), Linger: uint16(l.Linger)}
// 	return Setsockopt(fd, int32(level), int32(opt), (*byte)(unsafe.Pointer(&sys)), int32(unsafe.Sizeof(sys)))
// }

// fn SetsockoptInet4Addr(fd Handle, level, opt int, value [4]byte) (err error) {
// 	return Setsockopt(fd, int32(level), int32(opt), (*byte)(unsafe.Pointer(&value[0])), 4)
// }
// fn SetsockoptIPMreq(fd Handle, level, opt int, mreq *IPMreq) (err error) {
// 	return Setsockopt(fd, int32(level), int32(opt), (*byte)(unsafe.Pointer(mreq)), int32(unsafe.Sizeof(*mreq)))
// }
// fn SetsockoptIPv6Mreq(fd Handle, level, opt int, mreq *IPv6Mreq) (err error) { return EWINDOWS }

// fn Getpid() (pid int) { return int(getCurrentProcessId()) }

// fn FindFirstFile(name *uint16, data *Win32finddata) (handle Handle, err error) {
...
// fn FindNextFile(handle Handle, data *Win32finddata) (err error) {
// 	var data1 win32finddata1
...
// fn getProcessEntry(pid int) (*ProcessEntry32, error) {
...
// fn Fchdir(fd Handle) (err error)             { return EWINDOWS }
// fn Link(oldpath, newpath string) (err error) { return EWINDOWS }
// fn Symlink(path, link string) (err error)    { return EWINDOWS }

// fn Fchmod(fd Handle, mode uint32) (err error)        { return EWINDOWS }
// fn Chown(path string, uid int, gid int) (err error)  { return EWINDOWS }
// fn Lchown(path string, uid int, gid int) (err error) { return EWINDOWS }
// fn Fchown(fd Handle, uid int, gid int) (err error)   { return EWINDOWS }
...
// fn Geteuid() (euid int)                { return -1 }
// fn Getgid() (gid int)                  { return -1 }
// fn Getegid() (egid int)                { return -1 }
// fn Getgroups() (gids []int, err error) { return nil, EWINDOWS }
...
// fn LoadCreateSymbolicLink() error {
// 	return procCreateSymbolicLinkW.Find()
// }

// // Readlink returns the destination of the named symbolic link.
// fn Readlink(path string, buf []byte) (n int, err error) {
...
// 		s = utf16_to_string(p[data.SubstituteNameOffset/2 : (data.SubstituteNameOffset+data.SubstituteNameLength)/2])
...
// fn CreateIoCompletionPort(filehandle Handle, cphandle Handle, key uint32, threadcnt uint32) (Handle, error) {
...
// fn GetQueuedCompletionStatus(cphandle Handle, qty *uint32, key *uint32, overlapped **Overlapped, timeout uint32) error {
// 	var ukey uintptr
// 	var pukey *uintptr
// 	if key != nil {
// 		ukey = uintptr(*key)
...
// fn PostQueuedCompletionStatus(cphandle Handle, qty uint32, key uint32, overlapped *Overlapped) error {
...
// fn newProcThreadAttributeList(maxAttrCount uint32) (*_PROC_THREAD_ATTRIBUTE_LIST, error) {
// 	var size uintptr
...
// fn RegEnumKeyEx(key Handle, index uint32, name *uint16, nameLen *uint32, reserved *uint32, class *uint16, classLen *uint32, lastWriteTime *Filetime) (regerrno error) {
