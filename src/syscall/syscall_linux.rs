...
pub fn utimes_nano(path: &str, ts: &[Timespec; 2]) -> std::io::Result<()> {
    // pub fn UtimesNano(path: &str, ts: &[timespec; 2]) -> std::io::Result<()> {
    // 	if len(ts) != 2 {
    // 		return EINVAL
    // 	}
    // 	return utimensat(_AT_FDCWD, path, (*[2]Timespec)(unsafe.Pointer(&ts[0])), 0)
    let path_ = CString::new(path.as_bytes())?;
    let rc = unsafe { libc_::utimensat(libc_::AT_FDCWD, path_.as_ptr(), ts.as_ptr(), 0) };
    if rc == 0 {
        return Ok(());
    }
    let errno = libc_::get_errno();
    match errno {
        libc_::ENOENT => Err(std::io::Error::new(
