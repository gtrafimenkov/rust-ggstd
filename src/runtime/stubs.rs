...
fn fastrand_global() -> &'static atomic::AtomicU64 {
    use crate::encoding::binary::ByteOrder;
    use crate::encoding::binary::LITTLE_ENDIAN;

    static FR: OnceLock<atomic::AtomicU64> = OnceLock::new();
    fn init() -> atomic::AtomicU64 {
        let mut random_data = [0; std::mem::size_of::<u64>()];
        super::get_random_data(&mut random_data);
        let seed = LITTLE_ENDIAN.uint64(&random_data);
        let fr = atomic::AtomicU64::new(seed);
        _ = fr.fetch_add(0xa0761d6478bd642f, std::sync::atomic::Ordering::SeqCst);
        fr
    }
    FR.get_or_init(init)
}

/// fastrand returns a pseudorandom u32 value from the global fastrand
/// instance.  The global fastrand instance is seeded with a random
/// value on first use.
pub fn fastrand() -> u32 {
    // 	mp := getg().m
    ...
    // 		return uint32(hi ^ lo)
    let fr = fastrand_global().fetch_add(0xa0761d6478bd642f, std::sync::atomic::Ordering::SeqCst);
    let tmp = (fr as u128) * (fr ^ 0xe7037ed1a0b428db) as u128;
...
pub fn fastrandn(n: u32) -> u32 {
    // This is similar to fastrand() % n, but faster.
    // See https://lemire.me/blog/2016/06/27/a-fast-alternative-to-the-modulo-reduction/
    (((fastrand() as u64) * (n as u64)) >> 32) as u32
}
...
pub fn fastrandu() -> usize {
    let size = std::mem::size_of::<usize>();
    match size {
        4 => fastrand() as usize,
        8 => fastrand64() as usize,
        _ => panic!("unexpected usize size: {}", size),
    }
    // if std::mem::size::<usize>() == 4 {
    //     fastrand() as usize
    // } else {
    //     fastrand64()
    // }
