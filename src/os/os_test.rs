...
// fn size(name string, t *testing.T) int64 {
// 	file, err := Open(name)
// 	if err != nil {
...
// fn equal(name1, name2 string) (r bool) {
// 	switch runtime.GOOS {
// 	case "windows":
...
fn new_file(test_name: &str) -> TempFile {
    TempFile::new(test_name).unwrap()
...
// fn newDir(testName string, t *testing.T) (name string) {
...
// fn testReaddirnames(dir string, contents []string, t *testing.T) {
// 	file, err := Open(dir)
// 	if err != nil {
...
// fn testReaddir(dir string, contents []string, t *testing.T) {
// 	file, err := Open(dir)
// 	if err != nil {
// 		t.Fatalf("open %q failed: %v", dir, err)
// 	}
...
// fn benchmarkReaddirname(path string, b *testing.B) {
// 	var nentries int
// 	for i := 0; i < b.N; i++ {
// 		f, err := Open(path)
// 		if err != nil {
...
// fn benchmarkReaddir(path string, b *testing.B) {
// 	var nentries int
// 	for i := 0; i < b.N; i++ {
// 		f, err := Open(path)
// 		if err != nil {
...
// fn BenchmarkReaddirname(b *testing.B) {
// 	benchmarkReaddirname(".", b)
// }

// fn BenchmarkReaddir(b *testing.B) {
// 	benchmarkReaddir(".", b)
// }
...
// fn benchmarkStat(b *testing.B, path string) {
// 	b.ResetTimer()
// 	for i := 0; i < b.N; i++ {
// 		_, err := Stat(path)
// 		if err != nil {
...
// fn benchmarkLstat(b *testing.B, path string) {
// 	b.ResetTimer()
// 	for i := 0; i < b.N; i++ {
// 		_, err := Lstat(path)
// 		if err != nil {
...
// fn BenchmarkStatDot(b *testing.B) {
// 	benchmarkStat(b, ".")
// }

// fn BenchmarkStatFile(b *testing.B) {
// 	benchmarkStat(b, filepath.Join(runtime.GOROOT(), "src/os/os_test.go"))
// }

// fn BenchmarkStatDir(b *testing.B) {
// 	benchmarkStat(b, filepath.Join(runtime.GOROOT(), "src/os"))
// }

// fn BenchmarkLstatDot(b *testing.B) {
// 	benchmarkLstat(b, ".")
// }

// fn BenchmarkLstatFile(b *testing.B) {
// 	benchmarkLstat(b, filepath.Join(runtime.GOROOT(), "src/os/os_test.go"))
// }

// fn BenchmarkLstatDir(b *testing.B) {
// 	benchmarkLstat(b, filepath.Join(runtime.GOROOT(), "src/os"))
// }

// // Read the directory one entry at a time.
// fn smallReaddirnames(file *File, length int, t *testing.T) []string {
...
// 	readdirExpect := fn(n, want int, wantErr error) {
// 		t.Helper()
// 		fi, err := d.Readdir(n)
// 		if err != wantErr {
...
// 	readdirnamesExpect := fn(n, want int, wantErr error) {
// 		t.Helper()
...
// 	for _, fn := range []fn(int, int, error){readdirExpect, readdirnamesExpect, readDirExpect} {
// 		// Test the slurp case
// 		openDir()
// 		fn(0, 105, nil)
// 		fn(0, 0, nil)
...
// fn touch(t *testing.T, name string) {
// 	f, err := Create(name)
// 	if err != nil {
// 		t.Fatal(err)
// 	}
...
// 	*LstatP = fn(path string) (FileInfo, error) {
// 		if xerr != nil && strings.HasSuffix(path, "x") {
// 			return nil, xerr
// 		}
// 		return Lstat(path)
// 	}
// 	readDir := fn() ([]FileInfo, error) {
// 		d, err := Open(dir)
// 		if err != nil {
// 			t.Fatal(err)
// 		}
...
// 		return d.Readdir(-1)
// 	}
// 	mustReadDir := fn(testName string) []FileInfo {
// 		fis, err := readDir()
// 		if err != nil {
...
// fn TestRenameCaseDifference(pt *testing.T) {
...
// fn exec(t *testing.T, dir, cmd string, args []string, expect string) {
// 	r, w, err := Pipe()
// 	if err != nil {
// 		t.Fatalf("Pipe: %v", err)
// 	}
...
// fn checkMode(t *testing.T, path string, mode FileMode) {
// 	dir, err := Stat(path)
// 	if err != nil {
...
// fn checkSize(t *testing.T, f *File, size int64) {
// 	t.Helper()
// 	dir, err := f.Stat()
// 	if err != nil {
...
/// Use TempDir (via new_file) to make sure we're on a local file system,
/// so that timings are not distorted by latency and caching.
/// On NFS, timings can be off due to caching of meta-data on
/// NFS servers (Issue 848).
#[test]
fn test_chtimes_file() {
    let mut f = new_file("TestChtimes");
    f.f.write_all(b"hello, world\n").unwrap();
    test_chtimes(f.path.to_str().unwrap())
}
...
fn test_chtimes(name: &str) {
    // 	st, err := Stat(name)
    // 	if err != nil {
    // 		t.Fatalf("Stat %s: %s", name, err)
    // 	}
    // 	pre_stat := st
    let pre_stat = std::fs::metadata(name).unwrap();
    ...
    // 	}
    let at = Time::from_systime(&pre_stat.accessed().unwrap());
    let mt = Time::from_systime(&pre_stat.modified().unwrap());
    super::chtimes(
        name,
        &Time::from_unix(at.unix() - 1, 0),
        &Time::from_unix(mt.unix() - 1, 0),
    )
    .unwrap();
    ...
    // 	}
    // 	postStat := st
    let post_stat = std::fs::metadata(name).unwrap();

    let pre_at = pre_stat.accessed().unwrap();
    let pre_mt = pre_stat.modified().unwrap();
    let post_at = post_stat.accessed().unwrap();
    let post_mt = post_stat.modified().unwrap();
    ...
    assert!(
        pre_at > post_at,
        "AccessTime didn't go backwards; was={:?}, after={:?}",
        pre_at,
        post_at
    );
    assert!(
        pre_mt > post_mt,
        "ModTime didn't go backwards; was={:?}, after={:?}",
        pre_mt,
        post_mt
    );
}
...
// fn testWindowsHostname(t *testing.T, hostname string) {
...
// #[test]
// fn TestReadAtNegativeOffset() {
// 	f := new_file("TestReadAtNegativeOffset", t)
// 	defer Remove(f.Name())
// 	defer f.Close()
...
// #[test]
// fn TestWriteAtNegativeOffset() {
// 	f := new_file("TestWriteAtNegativeOffset", t)
// 	defer Remove(f.Name())
// 	defer f.Close()
...
// fn writeFile(t *testing.T, fname string, flag int, text string) string {
...
// #[test]
// fn TestStatDirWithTrailingSlash() {
...
// fn testDevNullFileInfo(t *testing.T, statname, devNullName string, fi FileInfo) {
...
// fn testDevNullFile(t *testing.T, devNullName string) {
...
// #[test]
// 		t.Run(fmt.Sprintf("length=%d", sz), fn() {
...
// fn testKillProcess(t *testing.T, processKiller fn(p *Process)) {
// 	testenv.MustHaveExec(t)
// 	t.Parallel()
...
// #[test]
// fn TestKillStartProcess() {
// 	testKillProcess(t, fn(p *Process) {
// 		err := p.Kill()
// 		if err != nil {
...
// 	f    fn(*File) error
// }{
// 	{"Chdir", fn(f *File) error { return f.Chdir() }},
// 	{"Close", fn(f *File) error { return f.Close() }},
// 	{"Chmod", fn(f *File) error { return f.Chmod(0) }},
// 	{"Chown", fn(f *File) error { return f.Chown(0, 0) }},
// 	{"Read", fn(f *File) error { _, err := f.Read(make([]byte, 0)); return err }},
// 	{"ReadAt", fn(f *File) error { _, err := f.ReadAt(make([]byte, 0), 0); return err }},
// 	{"Readdir", fn(f *File) error { _, err := f.Readdir(1); return err }},
// 	{"Readdirnames", fn(f *File) error { _, err := f.Readdirnames(1); return err }},
// 	{"Seek", fn(f *File) error { _, err := f.Seek(0, io.SeekStart); return err }},
// 	{"Stat", fn(f *File) error { _, err := f.Stat(); return err }},
// 	{"Sync", fn(f *File) error { return f.Sync() }},
// 	{"Truncate", fn(f *File) error { return f.Truncate(0) }},
// 	{"Write", fn(f *File) error { _, err := f.Write(make([]byte, 0)); return err }},
// 	{"WriteAt", fn(f *File) error { _, err := f.WriteAt(make([]byte, 0), 0); return err }},
// 	{"WriteString", fn(f *File) error { _, err := f.WriteString(""); return err }},
// }

// // Test that all File methods give ErrInvalid if the receiver is nil.
// #[test]
// fn TestNilFileMethods() {
...
// fn mkdirTree(t *testing.T, root string, level, max int) {
// 	if level >= max {
// 		return
// 	}
// 	level++
...
// fn testDoubleCloseError(t *testing.T, path string) {
// 	file, err := Open(path)
// 	if err != nil {
// 		t.Fatal(err)
// 	}
...
// fn isDeadlineExceeded(err error) bool {
// 	if !IsTimeout(err) {
// 		return false
// 	}
...
// #[test]
// fn TestOpenFileKeepsPermissions() {
// 	t.Parallel()
// 	dir := t.TempDir()
...
// 	if runtime.GOOS == "windows" {
// 		if err := filepath.WalkDir("./testdata/dirfs", fn(path string, d fs.DirEntry, err error) error {
// 			if err != nil {
// 				t.Fatal(err)
// 			}
// 			info, err := d.Info()
...
// 	fsys := os.DirFS(d)
// 	err := fs.WalkDir(fsys, ".", fn(path string, e fs.DirEntry, err error) error {
